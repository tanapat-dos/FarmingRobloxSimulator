...
-- The particle template is a baked child (script.Part.GoldMutationEffect) that only
...
local function getEffectTemplate(): Instance?
	...
	local template = part and part:FindFirstChild("GoldMutationEffect")
	if not template and not warnedMissing then
		warnedMissing = true
		warn("[Golden] Missing baked effect template script.Part.GoldMutationEffect")
	end
	return template
end
		...
		if seed_data.MultiHarvest.Value then
			...
			if found then
				for _,v in found:GetDescendants() do
					if v:IsA("Attachment") and v.Name == "GoldMutationEffect" then
						v:Destroy()
					end
				end
			end
