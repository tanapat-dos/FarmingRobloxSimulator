...
function mutationEffect.applyToolEffect(tool: Tool)
	...
	task.spawn(function()
		script.Part.GoldMutationEffect:Clone().Parent = tool.Handle
	end)
end
...
function mutationEffect.applyEffect(clientModel: Model, seed_data: Folder, serverFruitPart)
	if seed_data.MultiHarvest.Value then
		...
		if fruitModel then
			...
			if parent then
				script.Part.GoldMutationEffect:Clone().Parent = parent
			end
		end
	else
		script.Part.GoldMutationEffect:Clone().Parent = clientModel.PrimaryPart
	end
end
