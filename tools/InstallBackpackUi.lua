--[[
	Install backpack UI into the open place (no Rojo required).
	Paste into Studio Command Bar once, then Stop → Play.

	Creates ReplicatedStorage.Modules.BackpackPanelUi and updates MenuBarClient
	to add the 🎒 Bag button + B key panel.
]]
...
function BackpackPanelUi.isOpen()
	return panel ~= nil and panel.Visible
end

function BackpackPanelUi.setOpen(open)
	if not panel then
		return
	end
	panel.Visible = open
...
function BackpackPanelUi.mount(localPlayer)
	if mounted then
		return
	end
	mounted = true
...
local mod = modules:FindFirstChild("BackpackPanelUi")
if not mod then
	mod = Instance.new("ModuleScript")
	mod.Name = "BackpackPanelUi"
	mod.Parent = modules
end
mod.Source = MODULE_SOURCE

local menuBar = SPS:FindFirstChild("MenuBarClient")
if menuBar and menuBar:IsA("LocalScript") then
	local src = menuBar.Source
	if not string.find(src, "BackpackPanelUi", 1, true) then
		src = src:gsub(
		...
		src = src:gsub(
			...
			"%1BackpackPanelUi.mount(player)\n\n%2"
		)
		menuBar.Source = src
	end
else
	warn("[InstallBackpackUi] MenuBarClient missing — sync Rojo MenuBarClient.client.lua")
end

print("[InstallBackpackUi] Done. Save place (Ctrl+S), Stop Play, Play again. Use 🎒 Bag on left menu or press B.")
